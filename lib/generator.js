...
function load(fs, path, putils, _, child_process) {
  ...
  function Generator(dirName) {
    ...
    fs.mkdirsSync(self.genPath('.', 'config', 'static'));
    self.genPath('assets', ['scripts', 'styles', 'images']).forEach(function(assetPath) {
      fs.mkdirsSync(assetPath);
    });
    fs.mkdirsSync('views');

    fs.writeFileSync(self.genPath('.', 'assets', 'styles', 'seed.css'), CSS);
    fs.writeFileSync(self.genPath('.', 'assets', 'scripts', 'seed.js'), JS);
    fs.writeFileSync(self.genPath('.', 'views', 'index.html'), HTML);
