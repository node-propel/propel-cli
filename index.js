...
function load(program, _, path, Generator) {
  ...
  program
    ...
    .action(function(args) {
      loadApp().pkg({ configFile: args.configFile, fileProps: args.properties, interactive: args.noInteractive === undefined });
    });
  ...
  program
    ...
    .on('--help', function() {
      ...
      console.log('----');
      console.log('    pkg -n                    Disable interactive terminal session. Flag to allow below options');
      console.log('    pkg -f [version-files...] Supply list of files (comma separated string with no spaces) to apply version bump.\n' +
                  '                              Defaults to application.js configuration');
      console.log('    pkg -t <bump-type>        Bump files to specified bump type. Options: major, minor, patch. Defaults to application.js configuration');
    ...
    .action(function(args) {
      loadApp().bump({ files: args.versionFiles, type: args.bumpType, custom: args.custom, interactive: args.noInteractive === undefined });
    });
